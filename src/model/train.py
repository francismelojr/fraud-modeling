...
bayes_search = BayesSearchCV(model, space_lgbm, cv=5, scoring='roc_auc',
                                 n_iter= 3, verbose=0, random_state=0)

pipeline = Pipeline([('missing_indicator', missing_indicator_transformer),
                     ('imputer_N', imputer_N),
...
train_precision = metrics.precision_score(y_train, y_train_score > best_threshold)
...
with open(output_file, 'wb') as f_out:
    cloudpickle.dump(pipeline, f_out)
